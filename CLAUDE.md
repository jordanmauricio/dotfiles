...
- **`.chezmoi.toml.tmpl`** — prompts once for `name`, `email`, `workEmail`, `computerName`, `isWorkLaptop`, `gpgGit`, `opItemGPG`, `gpgPassphrase`. These keys are referenced by templates everywhere (`{{ .name }}`, `{{ .isWorkLaptop }}`, etc.). Adding a new template variable means adding a prompt here.
...
- **`dot_zshrc.tmpl`** — zsh entrypoint. Uses zinit for plugins, powerlevel10k (`dot_p10k.zsh`), fnm/nvm/bun/go/ruby/android PATH setup. It sources `configs/{functions,aliases,inputrc}` **directly from the chezmoi source dir** (`~/.local/share/chezmoi/configs/...`), so those files are not chezmoi-managed targets and don't need the `dot_` prefix — edits there take effect on next shell without `chezmoi apply`. Machine-local / work-specific env (e.g. the ws-opencode block) lives in untracked `~/.zshrc.local`, sourced at the end — never put work identifiers or secrets in the template. It also runs `gpg_cache` on startup, which signs into 1Password (`op`) and presets the GPG passphrase using `OPITEM`/`GPGPHRASE` from template data; `GH_TOKEN` is likewise pulled from `op`.
- **`private_dot_config/private_git/`** — git `config.tmpl` (identity from template data, 1Password as credential helper and SSH signing program, diff-so-fancy pager, a conditional include for `~/Projects/kramphub/`) and `ignore.tmpl` (global gitignore).
...
- **1Password template functions** (`onepasswordRead "op://vault/item/field"`, `onepassword`, `onepasswordItemFields`) — could replace the shell-side `op item get` calls in `dot_zshrc.tmpl` and the `gpgPassphrase`/`opItemGPG` prompts, keeping secrets out of the config file. Related: `chezmoi secret`, `encrypted_` + `age`/gpg for committed secrets.
