...
- **`.chezmoi.toml.tmpl`** — prompts once for `name`, `email`, `workEmail`, `computerName`, `isWorkLaptop` (bool, `promptBoolOnce`), `gpgGit` (SSH public key used for commit signing). These keys are referenced by templates everywhere (`{{ .name }}`, `{{ .isWorkLaptop }}`, etc.). Adding a new template variable means adding a prompt here.
- **`.chezmoidata/packages.yaml`** — the single source of truth for installed software: `taps`, `brews`, `casks`, `caskswork`, `caskspersonal`, `crates` (cargo), `nodes` (npm -g), `vscode` extensions. `run_onchange_install-packages-darwin.sh.tmpl` renders this into a `brew bundle` Brewfile on stdin plus `cargo install` / `npm install -g` loops and a single batched `code --install-extension` call (one invocation, not brew bundle `vscode` lines: back-to-back `code` calls race on `~/.vscode/extensions/extensions.json` and fail intermittently). `caskswork` vs `caskspersonal` is gated on `isWorkLaptop`. To add software, edit the YAML, not the script.
- **`.chezmoidata/agents.yaml`** — agent skill packs and which agents to install them to (`claude-code`, `codex`); applied by `run_onchange_after_agent-skills.sh.tmpl` via `npx skills`. Skills land as real copies in `~/.agents/skills` with `~/.claude/skills` symlinked to them, so editing `~/projects/skills` does **not** propagate until that script re-runs. Never use `npx skills --all` — it is shorthand for `--agent '*'` and silently overrides the target list, installing into ~50 agent dirs under `$HOME`. Plan and rationale: `docs/skills.md`.
- **`.chezmoitemplates/skills-flow.md`** — shared "Agent skills" block, inlined into both `private_dot_claude/CLAUDE.md.tmpl` and `private_dot_codex/AGENTS.md.tmpl` via `{{ template "skills-flow.md" . }}` (note: the template name includes the `.md` extension). `private_dot_codex/AGENTS.md.tmpl` also inlines the whole of `docs/tooling.md` with `{{ include }}`, because Codex has no `@import` syntax.
...
- **`dot_zshrc.tmpl`** — zsh entrypoint. zinit loads p10k synchronously and everything else in **turbo mode** (`zinit wait lucid for …`, after the first prompt; `compinit -C` via `zicompinit` — run `compinit` once after installing a tool with new completions). `brew shellenv`/`zoxide`/`fzf`/`brew command-not-found-init` go through **evalcache** (`_evalcache_clear` after upgrading those); `fnm env` is deliberately not cached (per-shell multishell path). fnm is the only node manager (reads `.nvmrc`/`.node-version`/`engines.node`); no ruby/rbenv, no GOROOT. `CLOUDSDK_PYTHON` is pinned to `python@3.13` for gcloud while `python3` stays latest. It sources `configs/{functions,aliases}` **directly from the chezmoi source dir** (`$CHEZMOI_HOME/configs/...`, i.e. this repo), so those files are not chezmoi-managed targets and don't need the `dot_` prefix — edits there take effect on next shell without `chezmoi apply`. Machine-local / work-specific env (e.g. the ws-opencode block) lives in untracked `~/.zshrc.local`, sourced at the end — never put work identifiers or secrets in the template. `GH_TOKEN` is fetched from 1Password lazily by a `gh()` wrapper on first use (`gh_token` exports it manually for other tools) so shell startup makes no `op` calls. Commit signing is SSH via 1Password (`gpg.format = ssh`); there is no GPG in use.
- **`private_dot_config/private_git/`** — git `config.tmpl` (identity from template data, 1Password as credential helper and SSH signing program, delta pager, `hunk` as difftool (`git difftool`), VS Code as mergetool (hunk has no merge mode), push aliases `p`/`pn` (no-verify)/`pf` (force-with-lease)/`pff` (force)/`pnf`, `includeIf gitdir:~/projects/kramp/` → `work.tmpl` which sets `{{ .workEmail }}`) and `ignore.tmpl` (global gitignore).
- **`dot_inputrc`** — readline settings for bash/python REPLs etc. (was wrongly sourced into zsh before).
- **`.github/workflows/test-macos.yml`** — weekly (Thu 21:00 UTC) bootstrap test on an ephemeral `macos-latest` runner: stages this repo at `~/projects/dotfiles`, stubs `op`, seeds the chezmoi prompt data, runs `chezmoi init && chezmoi apply`, then verifies the tool map, the rendered dotfiles and an interactive zsh. GUI casks are simulated (`brew install --cask --dry-run` + `brew fetch`) instead of installed unless dispatched with `cask_mode=install`. Failures open/comment a `dotfiles-ci` issue; a green run closes it. Helpers live in `.github/scripts/`. chezmoi ignores `.github` automatically (source entries starting with `.`).
...
- To disable a `run_*` script, rename it `skiprun_*` (matched by `.chezmoiignore`). `skiprun_` is not a chezmoi prefix — without the ignore entry chezmoi would copy it into `~` as a plain file. (Currently no script is disabled; `brew bundle` runs with `--no-upgrade` — upgrades are topgrade's job.)
