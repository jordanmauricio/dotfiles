...
- **`dot_zshrc.tmpl`** — zsh entrypoint. Uses zinit for plugins, powerlevel10k (`dot_p10k.zsh`), fnm/nvm/bun/go/ruby/android PATH setup. It sources `configs/{functions,aliases,inputrc}` **directly from the chezmoi source dir** (`$CHEZMOI_HOME/configs/...`, i.e. this repo), so those files are not chezmoi-managed targets and don't need the `dot_` prefix — edits there take effect on next shell without `chezmoi apply`. Machine-local / work-specific env (e.g. the ws-opencode block) lives in untracked `~/.zshrc.local`, sourced at the end — never put work identifiers or secrets in the template. `GH_TOKEN` is pulled from 1Password (`op item get`) on every shell start — that's the main startup cost. Commit signing is SSH via 1Password (`gpg.format = ssh`); there is no GPG in use.
